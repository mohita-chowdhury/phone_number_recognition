...
PATH = 'saved/models/speech_net_aug.pth.tar'
...
parser.add_argument('-e','--number_of_epochs',type=int,required=False,default=30,help='Number of epochs to run the model')
...
parser.add_argument('-gpu','--gpu_available',type=str,required=False,default='f',help='If this flag is t then a model will be trained using GPU')
parser.add_argument('-tr','--train',type=str,required=False,default='f',help='If this flag is t then model will be trained')
...
# speech_model = model.SpeechConv_2()
...
if train_ == 't':
    print('Training...')
    train(training_generator, number_of_epochs, batch_size, speech_model, optimizer, gpu_available)
...
test(test_generator, speech_model, gpu_available)
